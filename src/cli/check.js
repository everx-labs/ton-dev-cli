...
export async function resolveGiverParameters(): Promise<void> {
    const client = TONClientNodeJs.create({});
    try {
...
async function checkNetwork(
    server: string,
    verbose: boolean,
    report: (status: number, message: string) => void
): Promise<string> {
    const client = await TONClientNodeJs.create({
        servers: [server],
        log_verbose: verbose,
    });
    let result;
...
export async function checkNetworks(servers: string[], verbose: boolean): Promise<void> {
    await resolveGiverParameters();
    await Promise.all(servers.map((server) => {
        return checkNetwork(server, verbose);
    }));
